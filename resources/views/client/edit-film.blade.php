	...
	<div class="container">
		...
		<div class="agileinfo-news-top-grids">
			...
			<div class="col-md-8">
				...
				@endforeach
				@endif
				<form class="m-form m-form--fit m-form--label-align-right" id="form_add" action="{{ route('client.film.update', [$film->id]) }}" method="POST" enctype="multipart/form-data">
					@csrf
					<div class="m-portlet__body">
						...
						<div class="form-group m-form__group row">
							...
							<div class="col-lg-9 col-md-8 col-sm-12">
								<div class='input-group'>
									<input type='text' class="form-control" name="name" placeholder="Nhập tên" value="{{ $film->name }}"/>
								</div>
						...
						<div class="form-group m-form__group row">
							...
							<div class="col-lg-9 col-md-8 col-sm-12">
								<div class='input-group'>
									<input type='text' class="form-control" name="othername" placeholder="Nhập tên"  value="{{ $film->othername }}"/>
								</div>
								<span class="error error-required"></span>
							</div>
						</div>
						<div class="form-group m-form__group row">
							...
							<div class="col-lg-9 col-md-8 col-sm-12">
								<div class='input-group'>
									<select class="form-control" name="type"  value="{{ $film->type }}">
						...
						<div class="form-group m-form__group row">
							...
							<div class="col-lg-9 col-md-8 col-sm-12">
								<div class='input-group'>
									<select class="form-control" name="country_id" value="{{ $film->country->id }}">
						...
						<div class="form-group m-form__group row">
							...
							<div class="col-lg-9 col-md-8 col-sm-12">
								<div class='input-group'>
									<select class="form-control" name="director_id" value="{{ $film->director_id }}">
						...
						<div class="form-group m-form__group row">
							...
							<div class="col-lg-9 col-md-8 col-sm-12">
								<div class='input-group'>
									<input type='text' class="form-control" name="year" placeholder="Nhập năm phát hành"  value="{{ $film->year }}"/>
								</div>
								<span class="error error-required"></span>
							</div>
						</div>
							...
							<div class="form-group m-form__group row">
								...
								<div class="col-lg-9 col-md-8 col-sm-12">
									<div class='input-group date'>
										<input type="text" class="form-control" readonly="" placeholder="Select date" id="m_datepicker_2" name="release_date"  value="{{ $film->release_date }}">
						...
						<div class="more-info2">
							...
							<div class="col-lg-9 col-md-8 col-sm-12">
								<div class='input-group'>
									<input type='text' class="form-control" name="total_episodes" placeholder="Nhập tổng số tập"  value="{{ $film->total_episodes }}"/>
								</div>
								<span class="error error-required"></span>
							</div>
						</div>
						...
						<div class="form-group m-form__group row">
							...
							<div class="col-lg-9 col-md-8 col-sm-12">
								<div class='input-group'>
									<textarea name="description" id="" cols="30" rows="10"></textarea  value="{{ $film->description }}">
									</div>
									<span class="error error-required"></span>
								</div>
							</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		var arr_cates = [];
		@foreach($film->categories as $value)
		arr_cates.push(String({{$value}}));
		@endforeach
		$('.select2-categories').val(arr_cates);
		$('.select2-categories').trigger('change');
		var arr_actors = [];
		@foreach($film->actors as $value)
		arr_actors.push(String({{$value}}));
		@endforeach
		$('.select2-actors').val(arr_actors);
		$('.select2-actors').trigger('change');
		@if(!is_null($film->release_date))
		$('.more-info').css({'display' : 'block'});
		@endif
		$('select[name=type]').val({{$film->type}});
		$('select[name=country_id]').val({{$film->country->id}});
		$('select[name=director_id]').val({{$film->director_id}});
	});
	$("input[name=image]").fileinput({
			...
			maxFileCount: 1,
			initialPreview: [
			"<img class='kv-preview-data file-preview-image' src='{{ $film->image }}'>"
			],
			initialCaption: '{{ $film->image }}',
