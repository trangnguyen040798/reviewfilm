let table1 = $('#manage-slider-1_table_1').DataTable({
    "order": [[ 1, "desc" ]],
    "columns": [
    { "width": "5%" },
    { "width": "5%" },
...
let table2 = $('#manage-slider-2_table_1').DataTable({
    "order": [[ 1, "desc" ]],
});
            ...
            if (!response.error) {
                ...
                if (response.data.film) {
                    table2.row.add([
                        ...
                        response.data.id,
                        response.data.tag,
                        response.data.film.name,
                        response.data.film.user_name,
                        action_btn(response)
                        ]).node().id = 'manage-slider-2_' + response.data.id;
                    table2.draw( false );
                } else {
                    table1.row.add([
                        '<input type="checkbox" value="' + response.data.id + '">',
                        ...
                        action_btn(response)
                        ]).node().id = 'manage-slider-1_' + response.data.id;
                    table1.draw( false );
                }
            } else {
...
$(document).on('change', '#manage-slider-1_table_1 input[type=checkbox]', function() {
    ...
    if ($(this).attr('name') == 'checkall') {
        if ($(this).prop('checked')) {
            ...
            $('#manage-slider-1_table_1 tbody').find('input[type=checkbox]').prop({checked:true});
        } else {
            checked = false;
            $('#manage-slider-1_table_1 tbody').find('input[type=checkbox]').prop({checked:false});
        }
    } else {
        if (count == $('#manage-slider-1_table_1 tbody').find('input[type=checkbox]').length) {
            $('#manage-slider-1_table_1').find('input[name=checkall]').prop({checked:true});
        } else {
            $('#manage-slider-1_table_1').find('input[name=checkall]').prop({checked:false});
        }  
    }
    if (checked && count >0) {
        $('#manage-slider-1_table_1').parents('.dataTables_wrapper').prev().prev().prev('.multi-delete').css({'display' : 'block'});
        $('#manage-slider-1_table_1').parents('.dataTables_wrapper').prev().prev().prev('.multi-delete').find('.count-delete').text(count);
    } else {
        $('#manage-slider-1_table_1').parents('.dataTables_wrapper').prev().prev().prev('.multi-delete').css({'display' : 'none'});
...
$(document).on('change', '#manage-slider-2_table_1 input[type=checkbox]', function() {
    ...
    if ($(this).attr('name') == 'checkall') {
        if ($(this).prop('checked')) {
            ...
            $('#manage-slider-2_table_1 tbody').find('input[type=checkbox]').prop({checked:true});
        } else {
            checked = false;
            $('#manage-slider-2_table_1 tbody').find('input[type=checkbox]').prop({checked:false});
        }
    } else {
        if (count == $('#manage-slider-2_table_1 tbody').find('input[type=checkbox]').length) {
            $('#manage-slider-2_table_1').find('input[name=checkall]').prop({checked:true});
        } else {
            $('#manage-slider-2_table_1').find('input[name=checkall]').prop({checked:false});
        }  
    }
    if (checked && count >0) {
        $('#manage-slider-2_table_1').parents('.dataTables_wrapper').prev('.multi-delete').css({'display' : 'block', 'margin-bottom' : '20px'});
        $('#manage-slider-2_table_1').parents('.dataTables_wrapper').prev('.multi-delete').find('.count-delete').text(count);
    } else {
        $('#manage-slider-2_table_1').parents('.dataTables_wrapper').prev('.multi-delete').css({'display' : 'none'});
