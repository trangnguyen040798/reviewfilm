  ...
  $.ajax({
    ...
    success: function success(response) {
      if (!response.error) {
        ...
        table.row.add(['<input type="checkbox" value="' + response.data.id + '">', response.data.id, '<img src="' + response.data.image + '" alt="">', response.data.name, response.data.year, response.data.type, response.data.complete == 0 ? 'Chưa hoàn' : 'Hoàn', response.data.user.name, actionBtn(routeMangeVideo, response)]).node().id = 'film_' + response.data.id;
        table.draw(false);
      }
    },
  ...
  $.ajax({
    ...
    success: function success(response) {
      if (!response.error) {
        ...
        $('#EditItem').modal('hide');
        var routeMangeVideo = route('admin.film.manage-video.index', response.data.id);
        var newdata = ['<input type="checkbox" value="' + response.data.id + '">', response.data.id, '<img src="' + response.data.image + '" alt="">', response.data.name, response.data.year, response.data.type, response.data.complete == 0 ? 'Chưa hoàn' : 'Hoàn', response.data.user.name, actionBtn(routeMangeVideo, response)];
